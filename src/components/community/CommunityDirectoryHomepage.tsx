...
import { CommunityDirectoryRegistrationModal } from './CommunityDirectoryRegistrationModal';
...
export function CommunityDirectoryHomepage() {
  ...
  const handleRegistrationSuccess = () => {
    // Reload listings to show the new submission
    const loadData = async () => {
      try {
        const [recentListings, directoryStats] = await Promise.all([
          getRecentCommunityDirectoryListings(6),
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <Button 
              onClick={() => setIsRegistrationModalOpen(true)}
              className="bg-gradient-royal text-secondary"
            >
              Register Now
            </Button>
          </div>
